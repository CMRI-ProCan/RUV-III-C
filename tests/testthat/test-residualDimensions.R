test_that("Test residual dimensions", 
	{
		...
		for(k in 2:3)
		{
			for(version in c("R", "CPP"))
			{
				...
				result <- RUVIII_C(ruvInputData = copiedData, k = k, M = M, toCorrect = as.character(1:4), controls = as.character(15:20), withW = TRUE, filename = NULL, version = version)
				...
				expect_identical(unname(result$residualDimensions["4"]), -1L)
				expect_identical(unname(result$residualDimensions["5"]), 10L)

				result <- RUVIII_C_Varying(ruvInputData = copiedData, k = k, M = M, toCorrect = as.character(1:4), controls = as.character(15:20), withW = TRUE, filename = NULL, version = version)
				expect_identical(unname(result$residualDimensions["1"]), 9L)
				expect_identical(unname(result$residualDimensions["2"]), 9L)
				expect_identical(unname(result$residualDimensions["3"]), 8L)
				expect_identical(unname(result$residualDimensions["4"]), -1L)
